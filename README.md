# Hi, I'm Stefano!  

## 🧠 Lecturer | Researcher | Data Scientist 

I bridge **STEM and Social Sciences** to address real-world economic, financial, and societal challenges. My research focuses on **merging economics and AI**, including **applied econometrics, ML, NLP, time-series forecasting, and financial modeling**, with applied projects in **macroeconomics, credit risk, central bank communications**, and **Thai economic and societal issues**.  

On this GitHub, you'll find **academic projects, applied data science experiments, and tools** turning complex data into actionable insights.  

---

## 📚 Specialization & Interests  

- **Macroeconomic Forecasting & Time-Series** (Thailand-focused)  
- **AI, ML & NLP** applied to economics, finance, and policy  
- **Big Data & Decision Support Systems**  
- **Interdisciplinary Research** bridging data science with social and economic insights  

---

## 🛠 Tech Stack  

**Languages & Libraries:** Python, R, SQL, Pandas, Scikit-learn, TensorFlow, Statsmodels, Hugging Face  
**Big Data & Cloud:** AWS SageMaker, Spark  
**Data Engineering & MLOps:** MySQL, PostgreSQL, Docker, MLflow, Airflow  
**Visualization:** Matplotlib, Seaborn, Plotly  

---

## 📄 Selected Working Papers  

- **Mechanism-Based Intelligence (MBI)** – [arXiv](https://arxiv.org/pdf/2512.20688)  
- **Nowcasting Thai GDP with ML** – [Master's Thesis](https://www.researchgate.net/publication/395974492_Why_Both_Accuracy_and_Interpretability_Matter_Machine_Learning_for_Nowcasting_Thai_GDP)  
- **Forecasting Inbound Tourist Demand in Thailand with RNNs** – [ResearchGate](https://www.researchgate.net/publication/397020727_Forecasting_of_Inbound_Tourist_Demand_in_Thailand_Assessing_Recurrent_Neural_Networks)  
- **Sentiment Analysis of Thai Central Bank Press Releases** – [ResearchGate](https://www.researchgate.net/publication/390321864_Sentiment_Classification_of_Thai_Central_Bank_Press_Releases_Using_Supervised_Learning)  
- **Inflation Dynamics in the US (2021-2024)** – [ResearchGate](https://www.researchgate.net/publication/384766504_Understanding_Inflation_Dynamics_in_the_United_States_An_Analysis_of_2021-2024)  
- **Credit Risk Assessment with Bayesian Networks** – [ResearchGate](https://www.researchgate.net/publication/390577527_A_Decision_Support_System_for_Credit_Risk_Assessment_Using_Bayesian_Networks)  

*(More projects combining ML, econometrics, and applied research in my repositories)*  

---

## 📫 Connect with Me  

- 💼 **LinkedIn:** [Stefano Grassi](https://www.linkedin.com/in/steven-grassi/)  
- 📊 **ResearchGate:** [Stefano Grassi](https://www.researchgate.net/profile/Stefano-Grassi)  

---

🌱 *Exploring new ways to solve complex economic and societal challenges with data-driven insights.*
